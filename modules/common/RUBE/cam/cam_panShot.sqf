...
private ["_cam", "_target", "_distance", "_dist", "_height", "_angle", "_duration", "_pos", "_followTarget", "_heightASL", "_zBuffer", "_offsetMatrix", "_memPoint", "_t0", "_t1", "_fsDist", "_fsDir", "_fsHeight", "_absoluteAngle", "_originalTarget", "_fakeTarget", "_copy"];
...
private [
   "_t", "_t0", "_t1", "_tDist", "_tDir", "_tHeight", "_zBase",
   "_x", "_y", "_z", "_tgtZOffset", "_camZ", "_tgtZ", "_errZ"
];
...
while {_t1 < _duration} do
{
   ...
   if (((side _target) != sideLogic) && ((abs ((_pos select 0) - ((visiblePosition _target) select 0))) > 200)) then
   {
      _fakeTarget set [0, (createGroup sideLogic)];
      _fakeTarget set [1, ((_fakeTarget select 0) createUnit ["Logic", _pos, [], 0, "NONE"])];
      (_fakeTarget select 1) setVariable ["exitCamScript", false];
   ...
   // lags too much, so no sexy sleepy time for you :P 
   // ... or
   sleep 0.01;  
};
