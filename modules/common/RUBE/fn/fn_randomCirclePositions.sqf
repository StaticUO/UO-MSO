   ...
   Parameter(s):
    _this: parameters (array of array [key (string), value (any)])
           ...
           - optional:
             ...
             - "adjustPos" (scalar)
               - if greater than zero, the sample position may be adjusted
                 at max. the given distance to better fit isFlatEmpty criteria.
               - default: 0  
             ...
             - "blacklistDistance" (scalar)
                min. distance to stay away from points in the blacklist,
                aka a radius, if you like.
               - default = 500;  
             ...
             - "onShore" (boolean/int from 0 to 1)
...
private ["_thePositions", "_pos", "_numPos", "_distMin", "_distMax", "_dirMin", "_dirMax", "_objDistance", "_posDistance", "_adjustPos", "_maxGradient", "_gradientRadius", "_waterMode", "_onShore", "_roadDistance", "_locationDistance", "_locationDefinition", "_entities", "_i", "_maxIter", "_accepted", "_diceDir", "_diceDist", "_dicePos", "_blacklist", "_blacklistDistance"];
...
// read parameters
{
   switch (_x select 0) do
   {
      ...
      case "adjustPos": { _adjustPos = _x select 1; };
...
while {(count _thePositions) < _numPos} do
{
   ...
   _dicePos = [
      ((_pos select 0) + (_diceDist * (sin _diceDir))),
      ((_pos select 1) + (_diceDist * (cos _diceDir))),
      0
   ];
   
   // check if the position is safe
   if ((count (_dicePos isFlatEmpty[_objDistance, _adjustPos, _maxGradient, _gradientRadius, _waterMode, _onShore, objNull])) > 0) then
   {
      ...
      if (_blacklistDistance > 0) then
      {
         {
            if ((_dicePos distance _x) < _blacklistDistance) exitWith
            {
               _accepted = false;
            };
         } forEach _blacklist;
