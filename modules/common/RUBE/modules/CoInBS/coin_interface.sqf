				...
				if (
					...
					*/
					...
					_isFlat = (position _preview) isflatempty [
						((sizeof typeof _preview) * 0.1), //(sizeof typeof _preview) / 2,	//--- Minimal distance from another object
						0.05,//0,				//--- If 0, just check position. If >0, select new one
						0.79, //0.7,				//--- Max gradient
						((sizeof typeof _preview) * 0.79),//(sizeof typeof _preview),	//--- Gradient area
			...
			if (!isnull _preview && ((BIS_CONTROL_CAM_LMB && 65536 in (actionKeys "DefaultAction")) || {_x in (actionKeys "DefaultAction")} count BIS_CONTROL_CAM_keys > 0) && _color == _colorGreen) then {
				...
				*/
				...
				[_logic,_itemclass,_pos,_dir,_shift,_itemcost] spawn {
					...
					//--- Build				
					// *** [important change] Build needs to be done in onConstruct now ***
					_building = objNull;
					/*
					...
					*/
					...
					//[_logic, _building, _itemclass, _pos, _dir, _shift, _itemcost] spawn _code;
					[
						//["coin", _logic],
						["class", _itemclass],
						["position", _pos],
...
*/
		...
		if (!([_cashValues,_cashValuesOld] call bis_fnc_arraycompare) || _restart) then {
			...
			// don't show supplies (they're represented physically with ingame objects)
			/*
			...
			((uiNamespace getvariable "BIS_CONTROL_CAM_DISPLAY") displayctrl 112224) ctrlsetposition [_cashPos select 0,_cashPos select 1,_cashPos select 2,(_cashPos select 3) + _cashLines];
				...
				_arrayParams = [];
				{
					...
					if (_itemcategory < count _categories) then {
						...
						if (_itemcategory == _i/*_category*/) then {
							...
							_canAfford = 1; // BS: a building site costs nothing...
