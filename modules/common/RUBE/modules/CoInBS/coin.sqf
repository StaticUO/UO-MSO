...
/*
	...
	Author: Karel Moricky
	 ...
	 -> slightly modified for the RUBE library
		...
		if (isnil {player getvariable _BIS_COIN_idAction} && !isnull player) then 
		{
		  _player = player;
			...
			_action = player addaction [
				_description,
				"modules\common\RUBE\modules\CoInBS\coin_interface.sqf",
				_logic,
				1,
				false,
				false,
...
/*
    	_description = if (_name == '') then {localize 'str_coin_action'} else {localize 'str_coin_action' + ' - ' + _name};
	_action = _player addaction [_description,'RUBE\modules\CoIn\coin_interface.sqf',_logic,1,false,false,'',(_logic getVariable 'BIS_COIN_actionCondition')];
*/
