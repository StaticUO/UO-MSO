...
CLASS("UCD_obj_cachedObject")
	PROTECTED VARIABLE("code","properties"); // Instance of "CORE_obj_hashMap" object
	PUBLIC FUNCTION("","constructor") {
		private ["_prop"];
		_prop = ["new"] call CORE_obj_hashMap;
	...
	PUBLIC FUNCTION("array","constructor") {
		...
		if (_obj isKindOf "Man") then {
			...
			if ((vehicle _obj) != _obj) then {
				...
				["insert", ["unitVehPos", (_obj call CORE_fnc_unitVehPos)]] call _prop;
			};
	...
	if (alive _obj && {local _obj} && {!isPlayer _obj}) then {
		...
		while {!_cache} do {
			...
			if ((_cacheStats select 1) && // Allowed to cache in config
				...
				{([_obj] call CORE_fnc_closestPlayerDis) > (call (_cacheStats select 3))} // Farther away from players than the specified min distance
		...
		if (_cache) then {
			...
			if (_obj isKindOf "Man") then { // Unit
				_unitVehPos = _obj call CORE_fnc_unitVehPos;
				{ // forEach
			...
			if (_cacheObj) then { // Final caching of object
				[_group, "UCD_cachedObjects", (["new", [_obj, (_cacheStats select 2), (_cacheStats select 3)]] call UCD_obj_cachedAsset)] call CORE_fnc_push;
			};
		};
	};
};
...
UCD_fnc_cacheMonitor = {
	...
	while {!(isNil "_group") && {!isNull _group} && {(count (units _group)) >= 0} && {!isNil {_group getVariable ["UCD_monitorScript", nil]}}} do {
		...
		if ((count _cachedObjects) > 0) then {
			...
			private ["_minDis"];
			_minDis = [leader _group] call CORE_fnc_closestPlayerDis;
			if (_minDis >= 0) then {
				{ // forEach
